...
export const ToolCard: React.FC<ToolCardProps> = ({ 
  ...
  return (
    <div 
      className="group relative bg-white/95 backdrop-blur-xl border border-white/60 rounded-2xl p-8 hover:bg-white hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 cursor-pointer overflow-hidden"
      style={{ width: '430px', height: '305px' }}
      onClick={handleClick}
    >
      ...
      <div className="relative z-10 h-full flex flex-col">
        <div className="flex justify-start mb-6">
          {getAnimatedIcon(tool.id)}
        </div>
        
        <div className="text-center mb-6">
          <h3 className="text-3xl font-bold text-gray-900 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-cyan-400 group-hover:via-sky-500 group-hover:to-fuchsia-500 transition-all duration-300 leading-tight">
            {tool.title}
          </h3>
        </div>
        
        <p className="text-gray-600 leading-relaxed text-lg flex-1 mb-6 text-center">
          {tool.description}
        </p>
        
        <div className="flex items-center justify-between mt-auto">
          <span className="text-base text-cyan-600 font-semibold group-hover:text-cyan-500 transition-colors">
            {language === 'tr' ? 'Kullanmaya başla' : 'Get Started'}
          </span>
          <div className="relative">
            <svg 
              className="w-6 h-6 text-cyan-600 group-hover:text-cyan-500 group-hover:translate-x-3 transition-all duration-300" 
              fill="none" 
              stroke="currentColor" 
              viewBox="0 0 24 24"
            >
            ...
            <svg 
              className="absolute top-0 left-0 w-6 h-6 text-cyan-400 opacity-0 group-hover:opacity-100 group-hover:-translate-x-2 transition-all duration-300" 
              fill="none" 
              stroke="currentColor" 
              viewBox="0 0 24 24"
            >
